 ...
 * @param {Boolean} replaceTarget Should the src object replace the target value if it exists
 * @returns {Object} The merged settings object
 */
function mergeSettings(target, src, replaceTarget) {
    ...
    if (src != null && typeof src === 'object') {
      ...
      Object.keys(src).forEach(function(key) {
        ...
        if (srcValue == null || typeof srcValue !== 'object') {
          ...
          var isRelations = key === 'relations';
          var replace = isRelations;
          if (target == null || typeof target !== 'object' ||
            target[key] == null || replaceTarget) {
            // If target item is null, or not an object or should be replaced
            dst[key] = srcValue;
          } else {
            // mergeSettings() is called with replace true when the merge algorithm
            // should replace target's nested values and stop merging recursively
            // --> This is desired for model's relations for example
            dst[key] = mergeSettings(target[key], src[key], replace);
          }
        }
      });
    }
