...
DataSource.prototype.defineRelations = function(modelClass, relations) {
  ...
  var createListener = function(relationName, relation, targetModel, throughModel) {
  ...
  if (relations) {
    Object.keys(relations).forEach(function(rn) {
      ...
      if (r.polymorphic && r.type !== 'belongsTo' && !r.model) {
        throw new Error(g.f('Param "model" not specified for {{polymorphic}} %s relation: %s', r.type, rn));
      }
      if (r.polymorphic && r.type === 'belongsTo' && r.model) {
        throw new Error(g.f('{{polymorphic}} %s relation: %s does not expect param "model"', r.type, rn));
      }
      if (r.type === 'belongsTo' && typeof r.polymorphic === 'string') {
        throw new Error(g.f('Unexpected type for param "polymorphic" (String) in ' +
          '{{polymorphic}} %s relation: %s', r.type, rn));
      }
